...
#api endpoints
friends_api = 'https://api.twitter.com/1.1/friends/ids.json?screen_name='
...
while True:
    ...
    api_following = friends_api + my_id
    api_follower = followers_api + my_id

    while True:
        friend_headers, friend_data = client.request(api_following)
        ...
        api_following = friends_api + my_id + '&cursor=' + str(next_cursor)

    print('Looking for the ungrateful...')

    while True:
        friend_headers, friend_data = client.request(api_follower)
        friend_data = json.loads(friend_data.decode())
        try:
            next_cursor = friend_data['next_cursor']
        except:
        ...
        my_following_list += friend_data['ids']

        if not next_cursor:
            break

        api_follower = followers_api + my_id + '&cursor=' + str(next_cursor)

    print('Looking for my friends...')
    ...
    previous_allfriends = [x[0] for x in previous_allfriends]
    cur.close()
    conn.close()
    ...
    for my_friend in my_following_list:
        conn = sqlite3.connect(sql)
        cur = conn.cursor()
        cur.execute('INSERT OR IGNORE INTO twitter (user_id, request_time, is_friend) VALUES({}, 0, 1)'.format(my_friend))
        cur.execute('UPDATE twitter SET is_friend = 1 WHERE user_id = {}'.format(my_friend))
        conn.commit()
        cur.close()
        conn.close()
    ...
    for my_friend in my_friends_list:
        conn = sqlite3.connect(sql)
        cur = conn.cursor()
        cur.execute('INSERT OR IGNORE INTO twitter (user_id, request_time, is_friend) VALUES({}, 0, 0)'.format(my_friend))
        conn.commit()
        cur.close()
        conn.close()

    print('Saving my followings data...')
